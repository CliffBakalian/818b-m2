...
int BlobFinderUpdate(Model *mod, robot_t *robot);
...
extern "C" int Init(Model *mod, CtrlArgs *)
{
  ...
  */
  ...
  robot->blobfinder = dynamic_cast<ModelBlobfinder *>(mod);
	  //(ModelBlobfinder*)robot->pos->GetUnusedModelOfType("blobfinder");
  robot->blobfinder->AddCallback(Model::CB_UPDATE,model_callback_t(BlobFinderUpdate), robot);
...
int BlobFinderUpdate(ModelBlobfinder* blobmod, robot_t *robot){
  unsigned int blob_count = 0;
  const std::vector<ModelBlobfinder::Blob> blobs = blobmod->GetBlobs();
